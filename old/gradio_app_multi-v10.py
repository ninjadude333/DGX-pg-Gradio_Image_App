...
# Disable Gradio analytics to avoid pandas NDFrame issues
...
def apply_style(prompt: str, negative_prompt: str, style_name: str):
    ...
    - Prompt: we always append the style's prompt_suffix.
    - Negative prompt: we assume the textbox already contains what the user wants.
      The style preset can auto-fill it via the UI when empty, so we do NOT
      append negative_suffix again here to avoid double application.
    """
    ...
    full_negative = negative_prompt if negative_prompt.strip() else None
...
def clamp_for_heavy_models(width, height, batch_size):
    ...
    # keep batch_size as-is; just return it
...
def generate_image(
    ...
    # Save ALL images in the batch with descriptive filenames
    ...
    # Log this job and saved file names
...
print("Starting DGX SDXL Image Lab (loading initial model)...")
...
def on_style_change(style_name, current_negative):
    ...
    if current_negative is not None and current_negative.strip():
        # User already wrote something; keep it.
        return gr.update()
...
with gr.Blocks(title="DGX SDXL Image Lab v10") as demo:
    gr.Markdown(
        """
        # DGX SDXL Image Lab v10
