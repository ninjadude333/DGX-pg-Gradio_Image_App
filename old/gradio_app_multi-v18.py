...
def load_img2img_pipeline() -> bool:
    ...
    # PixArtSigmaPipeline is text-to-image only in this app (keeps v17 logic intact)
    if _current_model_key and MODEL_TYPES.get(_current_model_key) == "pixart":
...
def build_ui():
    """Build Gradio UI."""
    with gr.Blocks(title="SDXL DGX Image Lab v18") as demo:
        gr.HTML("<style>body { font-family: 'Segoe UI', sans-serif; }</style>")
        gr.Markdown("# SDXL DGX Image Lab v18 🚀")
