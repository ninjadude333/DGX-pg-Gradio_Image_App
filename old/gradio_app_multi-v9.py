...
def get_style_negative_text(style_name: str) -> str:
    """
    Return a clean negative prompt string for UI from the style preset.
    Strips leading comma/space.
    """
    style = STYLE_PRESETS.get(style_name, STYLE_PRESETS["None (raw prompt)"])
    ns = style.get("negative_suffix", "")
    ns = ns.lstrip(", ").strip()
...
def on_model_change(model_name):
    ...
    Note: Model and pipelines are global, shared across all browser sessions.
    """
    global pipe_txt2img, pipe_img2img, current_model_name
    ...
    try:
        ...
        # Make success message green and bold using HTML inside Markdown
        final_status = (
            f"<span style='color: green; font-weight: bold;'>"
            f"Loaded model {model_name} in {elapsed:.1f} seconds."
            f"</span><br><br>"
            f"- Repo: {model_id}<br>"
            f"- Offline mode: {HF_OFFLINE}<br>"
            f"- Pipelines: txt2img OK, img2img "
            f"{'OK' if img2img_pipe is not None else 'NOT AVAILABLE'}<br><br>"
...
# -----------------------------------------------------------------------------
# Aspect preset and style preset change handlers
# -----------------------------------------------------------------------------
...
def on_style_change(style_name):
    """
    When user selects a style preset, populate the negative prompt textbox
    with that style's default negative prompt text.
    """
    neg_text = get_style_negative_text(style_name)
    return gr.update(value=neg_text)

# -----------------------------------------------------------------------------
# Status for new sessions (so new tabs see current model)
# -----------------------------------------------------------------------------

def get_current_status():
    """
    Called on page load for each browser session.
    Shows whether a model is already loaded in the backend.
    """
    global current_model_name
    if current_model_name:
        return (
            f"<span style='color: green; font-weight: bold;'>"
            f"Backend already has model {current_model_name} loaded."
            f"</span><br><br>"
            "You can generate images immediately, or choose a different model."
        )
    else:
        return (
            "No model loaded yet.<br><br>"
            "Select a model from the dropdown above to start. "
            "The Generate & Save button will enable once the model is ready.<br><br>"
...
with gr.Blocks(title="DGX SDXL Image Lab v9") as demo:
    gr.Markdown(
        """
        # DGX SDXL Image Lab v9
        ...
        - Features:
          ...
          - Negative prompts (auto-filled by style presets, editable)
          - Seed (0 = random)
    ...
    with gr.Row():
        with gr.Column(scale=1):
            model_dropdown = gr.Dropdown(
                ...
                label="Model",
                info="Choose which SDXL model to use for generation.",
            )
            scheduler_dropdown = gr.Dropdown(
                choices=list(SCHEDULER_CLASSES.keys()),
                value="Default",
                label="Scheduler / Sampler",
                info="Default uses the model's built-in scheduler; others change the sampling method.",
            )
            style_dropdown = gr.Dropdown(
                choices=list(STYLE_PRESETS.keys()),
                value="Photoreal",
                label="Style preset",
                info="Applies extra prompt and negative-prompt tweaks for common looks.",
            )
            model_status = gr.Markdown(
                "No model loaded yet. Select a model above to start."
            )
        with gr.Column(scale=1):
            negative_prompt_in = gr.Textbox(
                lines=2,
                label="Negative prompt",
                placeholder="Things you DO NOT want (for example: ugly, blurry, extra fingers, cartoon)...",
            )
            seed_in = gr.Number(
                value=0,
                label="Seed (0 = random)",
        ...
        with gr.Column(scale=1):
            aspect_dropdown = gr.Dropdown(
                ...
                info="Select a fixed resolution or keep sliders / match reference image.",
            )
            width_in = gr.Slider(
                minimum=512,
                maximum=1280,
    ...
    # Extra descriptions for models and samplers
    gr.Markdown(
        """
        **Model hints:**
        - SDXL Base 1.0: General-purpose, good quality, slower.
        - SDXL Turbo: Very fast previews, fewer steps, sometimes less detail.
        - CyberRealistic / RealVis / Juggernaut: Photoreal-focused finetunes, heavier on VRAM.
        - DreamShaper XL Lightning: Fast, stylized, works well with fewer steps.
        - ZavyChroma XL: Colorful and stylized look.

        **Sampler hints:**
        - Default: Uses the sampler baked into the model.
        - Euler: Good all-rounder, often a bit sharper.
        - DPM++ 2M: High-quality sampler, can give smoother, more detailed results but slightly slower.
        """
    )
    ...
    # Style preset -> populate negative prompt textbox
    ...
    # When a new browser session connects, update status based on current_model_name
    demo.load(
...
# Queue is needed for generator-based UI updates (loading state, multiple users)
# Default queue behavior: jobs (including model loads and generations) run one at a time.
# This avoids race conditions such as changing models while an inference job is running.
demo.queue()
