...
# ------------------------------------------------------------
# Core generation (v13 with do_all_profiles)
# ------------------------------------------------------------
...
def _generate_single_profile(
    ...
    Returns: images, seeds, saved_paths, effective_negative
    """
    ...
    # Allow style to override scheduler & steps for this profile
    ...
    # Apply scheduler to pipe (no reloading model)
    ...
    for idx, img in enumerate(images):
        ...
        filename = f"{timestamp}_{slug_profile}_{slug_prompt}_seed{seed_val}_{idx+1:02d}.png"
    ...
    try:
        ...
        if do_all_profiles:
            ...
            for idx, prof_name in enumerate(profile_names):
                # For multi-profile, we do not use threaded batch to keep it simple/stable.
                images, seeds, paths, effective_negative = _generate_single_profile(
                    ...
                    height=height,
                    batch_size=batch_size,
                    base_seed=base_seed + idx * 1000000,
            ...
            profile_progress_text = (
                "Multi-profile run completed.<br>"
            ...
            # Single-profile behavior (v12-compatible)
            style_cfg = STYLE_PRESETS.get(style_name, STYLE_PRESETS["None / Raw"])
            styled_prompt = _append_style_suffix(prompt or "", style_cfg.get("prompt_suffix", ""))
            effective_negative = _append_style_suffix(
                negative_prompt_in or "",
...
def build_ui():
    with gr.Blocks() as demo:
        ...
        gr.Markdown("# SDXL DGX Image Lab v13 🚀")
        gr.Markdown(
        ...
        with gr.Row():
            do_all_profiles_checkbox = gr.Checkbox(
                label="Do ALL Profiles (v13 multi-style batch)",
                value=False,
            )
