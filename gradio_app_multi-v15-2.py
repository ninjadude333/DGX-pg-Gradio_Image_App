...
def _load_img2img_pipeline(model_key: str, scheduler_name: str) -> Tuple[bool, float, str]:
    ...
    Fix:
    - Avoid deadlock by never calling _load_txt2img_pipeline while holding _state_lock.
    - Force local-only HF loading (local_files_only=True).
    - Extra logging so we can see exactly where things break if they do.
    """
    global _img2img_pipe, _CURRENT_MODEL_KEY, _CURRENT_MODEL_ID, _CURRENT_SCHEDULER

    model_id = AVAILABLE_MODELS[model_key]

    # 1) Quick reuse check under the lock
    with _state_lock:
        if (
            ...
            print(
                f"[LOAD][IMG2IMG] Reusing already loaded img2img pipeline "
                f"for model={model_key} ({model_id}), scheduler={scheduler_name}",
                flush=True,
            )
            return False, 0.0, "Img2Img pipeline already loaded and ready."

    # 2) Ensure txt2img is loaded OUTSIDE the lock to avoid re-entrancy deadlock
    _load_txt2img_pipeline(model_key, scheduler_name)

    # 3) Actually build the Img2Img pipeline (no lock needed here)
    t0 = time.time()
    print(
        f"[LOAD][IMG2IMG] About to create AutoPipelineForImage2Image "
        f"for model={model_key} ({model_id})",
        flush=True,
    )
    ...
    try:
        pipe = AutoPipelineForImage2Image.from_pretrained(
            ...
            torch_dtype=torch.float16 if DEVICE == "cuda" else torch.float32,
            cache_dir=HF_CACHE_DIR,
            local_files_only=True,  # stay fully offline
        )
        print(
            f"[LOAD][IMG2IMG] from_pretrained() returned for model={model_key}",
            flush=True,
        )
        ...
        # Do not keep a half-initialized pipeline around
        with _state_lock:
            _img2img_pipe = None
        return False, 0.0, msg + " Falling back to Txt2Img for this job."
    ...
    # 4) Publish to globals under the lock
    with _state_lock:
        _img2img_pipe = pipe
        _CURRENT_MODEL_KEY = model_key
        _CURRENT_MODEL_ID = model_id
...
def _generate_single_profile(
    ...
    if mode_used == "img2img" and init_image is not None:
        print(
            f"[IMG2IMG] _generate_single_profile: model={model_key}, style={style_name}, "
            f"batch={batch_size}, size={width}x{height}, seed_base={base_seed}",
            flush=True,
        )
...
def generate_images(
    ...
    # Extra Img2Img logging and lazy-load
    if mode == "Image to Image":
        if init_image is None:
            warnings.append(
                "Img2Img mode selected but no input image provided. Falling back to Txt2Img."
            )
            print(
                f"[IMG2IMG][FALLBACK] No init image provided; request will run as txt2img. "
                f"model={model_key}, style={style_name}, size={width}x{height}, batch={batch_size}",
                flush=True,
            )
        else:
            # Shape for logging
            if isinstance(init_image, np.ndarray):
                ...
                shape_str = "PIL"
            print(
                f"[IMG2IMG] Request received: model={model_key}, style={style_name}, "
                f"size={width}x{height}, batch={batch_size}, strength={strength}, "
                f"init_image_shape={shape_str}",
                flush=True,
            )
            ...
            # v15.1: log a "job started" entry to jobs.log for Img2Img
            ...
            start_entry = {
                ...
                "batch_size": batch_size,
                "seed_base": seed,
                "info": "Img2Img job started (preview entry, final entry logged after completion).",
            }
            _log_job(start_entry)
    ...
    try:
        ...
        # ---------------------------
        # All profiles for selected model
        # ---------------------------
        ...
        # ---------------------------
            ...
            if mode == "Image to Image" and init_image is not None and _img2img_pipe is not None:
                ...
                if isinstance(init_image, np.ndarray):
                    ...
                    shape_str = "PIL"
                print(
                    f"[IMG2IMG] Single-profile: model={model_key}, style={style_name}, "
                    f"size={width}x{height}, batch={batch_size}, strength={strength}, "
                    f"init_image_shape={shape_str}",
                    flush=True,
                )
...
def build_ui():
    with gr.Blocks() as demo:
        ...
        gr.Markdown("# SDXL DGX Image Lab v15.1 🚀")
        gr.Markdown(
            "Single-GPU SDXL image lab running on DGX. "
            "Models are loaded from the local HuggingFace cache at `/root/.cache/huggingface`.\n\n"
            "**v15.1:** Extra verbose logging for Img2Img (job-start log entries, detailed request prints)."
        )
        ...
        with gr.Row():
            ...
            do_all_models_profiles_checkbox = gr.Checkbox(
                label="Do ALL Models × Profiles (grid, overnight mode)",
                value=False,
            )
